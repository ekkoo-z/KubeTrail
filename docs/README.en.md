...
- **Node-level execution validation**: controlled templates can create privileged containers and enter the target Node host view to validate hostPath, hostPID, runtime socket, and node-level permission exposure.
...
Note: C2 orchestration integrates with external operator-owned infrastructure through MCP. This repository keeps only the integration entry points and invocation constraints; it does not include a C2 server implementation.
...
Baseline collection from inside a Pod:

```bash
./kubetrail-server -m safe -o dbus.json
```

Local kubeconfig test:

```bash
./kubetrail-server -m safe -k ~/.kube/config -o dbus.json
```

Reduce sensitive material egress risk before sharing results or calling Agent:
...
Focus on local privilege escalation and container escape surface:

```bash
./kubetrail-server -m safe -s lpe,escape -o local-attack-surface.json
```

Run full mode when active validation is authorized:

```bash
./kubetrail-server -m full -o full.json
```

Export visible ServiceAccount token Secrets and audit each token:
...
| Mode | Behavior | Possible side effects |
| --- | --- | --- |
| `safe` | Default mode. Local read-only collection plus Kubernetes API read-only collection. | API read/list/review requests. |
| `full` | Includes `safe`, then adds DNS service discovery, cloud metadata HTTP probes, Admission dry-run, and syscall probes. | DNS queries, HTTP requests, dry-run create requests, and syscall attempts. |

Start with `safe` to confirm authorization boundaries, audit expectations, and output size, then run `full` only when needed.
...
1. Baseline from a container foothold
   ...
   Upload and run `kubetrail-server` from an acquired container shell, debug container, or test Pod. Start with `safe` mode to build baseline evidence.
...
4. Active validation pre-check
   ...
   `full` is useful for validating cloud metadata reachability, Admission constraints, service discovery exposure, and local syscall behavior, but it should not be the first indiscriminate scan.
...
5. Visible ServiceAccount token audit
   ...
   `secret-audit.json` contains raw tokens and must be stored, transmitted, and destroyed as highly sensitive credential evidence.
...
6. Agent-assisted analysis and EXP planning
   ...
   ```bash
   cd apps/agent
   npm run chat -- --input ../../dbus.json --message "List the highest-value attack surfaces, validation paths, and cleanup guidance with evidence."
   npm run exp -- list
   ```
